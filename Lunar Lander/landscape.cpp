...

{
  ...
  v = position - segHead;   // Let v be the vector from h to p
  u = segTail - segHead;    // Let u be the vector from h to t

  float x;

  x = (v * u)/(u * u);

  if (x < 0) {      // Projection outside range on head end
    return segHead;
  }

  else if (x > 1) { // Projection outside range on tail end
    return segTail;
  }

  else {            // Projection inside range
    return segHead + x * u;
    // x is the fraction of the distance you travel
    // along u to get to the desired closest point
  }
...
// Find the height of the terrain at a particular x position.
//
// You'll need to use linear interpolation.
//
// Used in world.cpp to report the altitude.


float Landscape::findHeightAtX( float x )

{
  // YOUR CODE HERE
  // Find height of land segment at said x value

  // Find line at x:
  float slope, yIntercept;

  for (int i = 0; i < numVerts - 1; i++) {
    if (landscapeVerts[2*i] < x && x < landscapeVerts[2*(i+1)]){ //segtail.x < x < seghead.x
      slope = (landscapeVerts[2*(i+1)+1] - landscapeVerts[2*i+1]) / (landscapeVerts[2*(i+1)] - landscapeVerts[2*i]); //slope = (y2-y1) / (x2-x1)
      yIntercept = landscapeVerts[2*i+1] - slope * landscapeVerts[2*i]; //intercept = y1 - slope * x1
    }
  }

  // Account for size of lander, position is measured
  // from centre of lander, lander width is 10m,
  // therefore 5 accounts for this
  float heightAtX = slope * x + yIntercept + 5;
